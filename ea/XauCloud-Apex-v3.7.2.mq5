...
bool CloudSync(){
   ...
   if(!Http("POST","/api/cloud/monitor/heartbeat",b,r,code,err)){
      if(code==401||code==403){
         ...
         g_cloudExplicitDenied=true;g_cloudLastStatus=reason;C.armed=false;
         Print("APEX LICENSE DENIED BY XAUCLOUD HEARTBEAT | http=",code," | reason=",reason);
      }else{
   ...
   if(!Http("GET",configPath,"",r,code,err)){
      string reason=js(r,"reason","LICENSE_DENIED");
      if(code==401||code==403){g_cloudExplicitDenied=true;g_cloudLastStatus=reason;C.armed=false;Print("APEX LICENSE DENIED BY XAUCLOUD | http=",code," | reason=",reason);}
...
double FloorVol(double v){
 double mn=SymbolInfoDouble(_Symbol,SYMBOL_VOLUME_MIN);
 double mx=SymbolInfoDouble(_Symbol,SYMBOL_VOLUME_MAX);
 double st=VolStep();
 if(v<=0||mx<=0||st<=0)return 0;
 v=MathMin(v,mx);
 double n=MathFloor(v/st+1e-9)*st;
 n=NormalizeDouble(n,VolDigits());
 if(n+1e-9<mn)return 0;
 return n;
}
double LargestVolumeWithinMargin(int dir,double price,double money,double hi){
 if(hi<=0||money<=0||price<=0)return 0;
 ENUM_ORDER_TYPE t=dir>0?ORDER_TYPE_BUY:ORDER_TYPE_SELL;
 double top=FloorVol(hi),m=0;
 if(top>0&&OrderCalcMargin(t,_Symbol,top,price,m)&&m<=money)return top;
 double lo=0,up=hi;
 for(int i=0;i<40;i++){
...
double VolumeForMargin(int dir,double pct){
 ...
 double mx=SymbolInfoDouble(_Symbol,SYMBOL_VOLUME_MAX);
 double mn=SymbolInfoDouble(_Symbol,SYMBOL_VOLUME_MIN);
 if(free<=0||price<=0||mx<=0)return 0;

 double usePct=clamp(pct,.1,100.0);
 double budget=free*usePct/100.0;

 // Full currently affordable capacity first.
 double fullCapacity=LargestVolumeWithinMargin(dir,price,free,mx);
 if(fullCapacity<=0)return 0;

 // Apply the configured NORMAL percentage to capacity.
 double byCapacityPct=FloorVol(fullCapacity*usePct/100.0);

 // Also require the same percentage of free margin to afford it.
 double byMarginBudget=LargestVolumeWithinMargin(dir,price,budget,mx);

 double v=0;
 if(byCapacityPct>0&&byMarginBudget>0)v=MathMin(byCapacityPct,byMarginBudget);

 // Broker minimum only when that SAME configured budget can independently afford it.
 if(v<=0&&mn>0){
   ENUM_ORDER_TYPE t=dir>0?ORDER_TYPE_BUY:ORDER_TYPE_SELL;
   double mmin=0;
   if(OrderCalcMargin(t,_Symbol,mn,price,mmin)&&mmin<=budget)
      v=NormalizeDouble(mn,VolDigits());
 }
 return v;
}
...
bool SetMasterSL(double newSL,string reason){
 ...
 bool accepted=sent&&(rc==TRADE_RETCODE_DONE||rc==TRADE_RETCODE_NO_CHANGES);
 bool applied=false;
 if(accepted&&PositionSelectByTicket(masterTicket)){
   double actual=PositionGetDouble(POSITION_SL);
   applied=MathAbs(actual-newSL)<=MathMax(_Point*2.0,1e-8);
 }
 if(applied){
   firstSLPrice=newSL;
   SaveState();
 ...
 Emit("MASTER_SL_MOVE_FAIL",StringFormat(",\"stage\":%d,\"requestedSL\":%.5f,\"retcode\":%d,\"error\":%d,\"reason\":\"%s\"",masterGuardStage,newSL,rc,GetLastError(),reason));
 return false;
}
bool OpenLayer(int dir,double score,string why){
 ...
 int digits=(int)SymbolInfoInteger(_Symbol,SYMBOL_DIGITS);
 double sl=0;
 if(firstNormal&&InpNormalFixedSLGoldMove>0)
   sl=NormalizeDouble(dir>0?reqPrice-InpNormalFixedSLGoldMove:reqPrice+InpNormalFixedSLGoldMove,digits);

 ResetLastError();
 bool sent=dir>0?trade.Buy(vol,_Symbol,0,sl,0,com):trade.Sell(vol,_Symbol,0,sl,0,com);
 uint rc=trade.ResultRetcode();
 bool accepted=sent&&(rc==TRADE_RETCODE_DONE||rc==TRADE_RETCODE_DONE_PARTIAL);
 if(!accepted){
   Emit("ORDER_FAIL",StringFormat(",\"retcode\":%d,\"error\":%d,\"volume\":%.4f",rc,GetLastError(),vol));
   return false;
 }

 double afterVol=BasketVolume();
 double st=VolStep();
 for(int i=0;i<5&&afterVol<=beforeVol+st*0.25;i++){
   Sleep(10);
   afterVol=BasketVolume();
 }
 double actualAdded=afterVol-beforeVol;
 if(actualAdded<=st*0.25){
   Emit("ORDER_UNCONFIRMED",StringFormat(",\"retcode\":%d,\"requestedVolume\":%.4f,\"beforeVolume\":%.4f,\"afterVolume\":%.4f",rc,vol,beforeVol,afterVol));
   return false;
 }

 layers++;
 lastAdd=trade.ResultPrice()>0?trade.ResultPrice():(dir>0?SymbolInfoDouble(_Symbol,SYMBOL_ASK):SymbolInfoDouble(_Symbol,SYMBOL_BID));
 if(firstNormal){
 ...
 SaveState();
 Emit("LAYER_OPEN",StringFormat(",\"layer\":%d,\"requestedVolume\":%.4f,\"actualAddedVolume\":%.4f,\"marginPct\":%.2f,\"score\":%.2f,\"reason\":\"%s\",\"price\":%.5f,\"sl\":%.5f",layers,vol,actualAdded,pct,score,why,lastAdd,sl));
 return true;
}
...
void Manage(){
 ...
 if(C.accountProfile=="NORMAL"&&camp&&masterTicket>0&&!MasterPositionExists()){
   if(n>0&&!CloseAll()){Emit("BASKET_CLOSE_INCOMPLETE",",\"reason\":\"MASTER_FIRST_TRADE_GONE\"");return;}
 ...
 if(C.accountProfile=="NORMAL"&&InpRecoveryExitEnabled&&masterTicket>0&&
   ...
   if(originalSLDist>0){
     ...
     if(recoveredToEntry){
       if(n>0&&!CloseAll()){Emit("BASKET_CLOSE_INCOMPLETE",",\"reason\":\"RECOVERY_TO_ENTRY_EXIT\"");return;}
...
// v3.4: once whole Apex basket reaches the configured campaign-profit threshold,
 ...
 if(C.accountProfile=="NORMAL"&&masterTicket>0&&firstEntryPrice>0&&firstSLPrice>0){
   ...
   if(masterGuardHit){
     if(n>0&&!CloseAll()){Emit("BASKET_CLOSE_INCOMPLETE",",\"reason\":\"MASTER_INPUT_FIXED_SL_HIT\"");return;}
...
int OnInit(){
   ...
   Print("APEX_READY ",APEX_VERSION,
      ...
      " | v3.7.1 SIGNAL LOGIC PRESERVED | operational fixes only v2");
