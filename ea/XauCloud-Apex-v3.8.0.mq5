...
#define APEX_VERSION       "XauCloud-Apex_v3.8.0-AstraFix"
#define APEX_BUILD_ID      "3.8.0"
...
//   * every telemetry Emit() is queued locally and never blocks a decision;
...
void QueueEvent(const string body)
  {
   if(g_eventQCount>=APEX_EVENTQ_MAX)
     {  // drop oldest; telemetry loss is preferable to unbounded memory
...
// Emit() NEVER performs I/O. It queues. APEX-AUDIT-002.
...
void FlushEventQueue()
  {
   ...
   if(Http("POST","/api/cloud/apex/event",b,r,code,err))
     {g_eventQHead=(g_eventQHead+1)%APEX_EVENTQ_MAX;g_eventQCount--;CloudSuccess();}
   else
     {
      ...
      if(code>=400&&code<500&&code!=429){g_eventQHead=(g_eventQHead+1)%APEX_EVENTQ_MAX;g_eventQCount--;}
     }
  }
...
string ConfigCanonical(const Config &x)
  {
   return StringFormat(
     ...
     "|%.4f|%.4f|%.4f|%.4f|%s|%.4f|%.4f|%.4f|%.4f|%s|%.4f|%s|%.4f|%.4f|%.4f|%.4f",
...
string ConfigToJson(const Config &x)
  {
   return StringFormat(
     ...
     "\"marginReservePct\":%.6f,\"commandRevision\":%I64d,\"denied\":%s,\"deniedReason\":\"%s\"}",
     ...
     x.maxBasketLots,x.minMarginLevelPct,x.marginReservePct,g_cloudLastCommandRevision,
     BoolJson(g_cloudExplicitDenied),g_cloudDeniedReason);
  }
...
bool CloudSync()
  {
   ...
   string b=StringFormat(
      ...
      "\"account_connected\":%s,\"symbol_trade_mode\":%d,\"ea_active\":true,\"bot_state\":\"APEX\"}",
...
string SizingJson(const SizingDecision &d,int layerIndex)
  {
   return StringFormat(
     ...
     "\"marginBinding\":%s,\"orderCheckRetcode\":%d,\"sizingBlock\":\"%s\"",
     C.accountProfile,layerIndex,d.pct,AccountInfoInteger(ACCOUNT_LEVERAGE),
...
void PrintSizing(const SizingDecision &d,int layerIndex,int dir)
  {
   PrintFormat("APEX SIZING | profile=%s layer=L%d dir=%d pct=%.2f | leverage=1:%I64d freeMargin=%.2f budget=%.2f"
               ...
               " | requested=%.4f FINAL=%.4f marginAtFinal=%.2f orderCheck=%d block=%s",
               C.accountProfile,layerIndex,dir,d.pct,AccountInfoInteger(ACCOUNT_LEVERAGE),d.freeMargin,d.budget,
               ...
               d.requested,d.finalVolume,d.marginAtFinal,d.checkRetcode,
   ...
   while(true)
     {
      ...
      if(e.cls==EXEC_FILLED||e.cls==EXEC_PARTIAL) break;
      ...
      for(int i=PositionsTotal()-1;i>=0;i--)
        {
         ulong t=PositionGetTicket(i);
         if(t&&PositionGetString(POSITION_SYMBOL)==_Symbol&&PositionGetInteger(POSITION_MAGIC)==InpMagic)
           {any=true;trade.PositionClose(t);}
        }
      if(!any)break;
      Sleep(100);
     }
...
void Manage()
  {
   ...
   if(C.accountProfile=="NORMAL"&&masterTicket>0&&!MasterPositionExists())
     {
   ...
   if(C.accountProfile=="NORMAL"&&P.recoveryExitEnabled&&masterTicket>0&&
   ...
   if(C.accountProfile=="NORMAL"&&P.masterBreakEvenEnabled&&masterTicket>0&&
   ...
   if(C.accountProfile=="NORMAL"&&P.profitRatchetEnabled&&cycleStart>0&&
...
int OnInit()
  {
   ...
   if(!IsTester()) ReconcileAgainstBroker();
   ...
   Print("APEX_READY ",APEX_VERSION,
      ...
      " | strategy unchanged from v3.7.1");
...
void OnDeinit(const int r)
  {
   ...
   if(!IsTester()){SaveState();ReleaseLease();}
  }
   ...
   if(!IsTester())
     {
      ...
      if(!held&&!g_observerOnly)
        ...
        {g_observerOnly=false;Print("APEX ACQUIRED INSTANCE LEASE | resuming management");}
     }
