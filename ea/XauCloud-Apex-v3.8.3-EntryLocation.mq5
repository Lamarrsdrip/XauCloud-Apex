//+------------------------------------------------------------------+
//|  XauCloud Apex v3.8.3 "EntryLocation"                             |
//|                                                                   |
//|  Strategy skeleton is still:                                      |
//|      impulse -> liquidity sweep -> rejection -> displacement      |
//|      -> ENTER THE ORIGIN BOX (engulfing/displacement retest)      |
//|      -> pyramid adds only while the basket is floating-profitable |
//|         AND the original origin has not been reclaimed            |
//|      -> basket exit on target / ratchet / master SL / recovery.   |
//|                                                                   |
//|  v3.8.3 repairs ENTRY LOCATION. It does not change sizing,        |
//|  leverage, UNLIMITED allocation, or add any new risk cap.         |
//|                                                                   |
//|  What changed vs v3.8.2:                                          |
//|    - A confirmed setup stores an origin box (the displacement     |
//|      candle). Confirmation is not permission to submit.           |
//|    - Waiting for the retest is legal. A newer M1 than the origin  |
//|      bar is the retest, not a stale trigger.                      |
//|    - Submit only while live price is inside the origin box and    |
//|      the swept extreme has not been reclaimed.                    |
//|    - A WATCHING setup that prints a new extreme UPDATES the       |
//|      watched high/low (same thesis). A CONFIRMED setup that       |
//|      prints a new extreme DIES and does not re-arm on that bar.   |
//|    - Continuation/failed-pullback adds use the CAMPAIGN origin    |
//|      and reclaim the original extreme.                            |
//|                                                                   |
//|  v3.8.2 CapacityTruth sizing is unchanged.                        |
//+------------------------------------------------------------------+
#property copyright "XauCloud Apex"
#property version   "3.830"
#property strict
#property description "ApexStack: XAUUSD exhaustion/reversal with origin-box retest entries and aggressive profit-side pyramiding"

#include <Trade/Trade.mqh>
CTrade trade;

#define APEX_VERSION       "XauCloud-Apex_v3.8.3-EntryLocation"
#define APEX_BUILD_ID      "3.8.3"
...
struct Setup
  {
   ...
   string     bosKind;
   double     originHigh,originLow,originClose;
...
void SetupReset(string reason)
  {
   ...
   S.originHigh=0;S.originLow=0;S.originClose=0;S.originBarTime=0;
...
Snap Observe()
  {
   ...
   s.originHigh=0;s.originLow=0;s.originClose=0;s.originBarTime=0;s.inLocation=false;
   ...
   bool haveSwingH=false,haveSwingL=false;
   for(int i=10;i<79;i++)
     {
      if(m1[i].high>m1[i-1].high&&m1[i].high>m1[i+1].high){ph=MathMax(ph,m1[i].high);haveSwingH=true;}
      if(m1[i].low<m1[i-1].low&&m1[i].low<m1[i+1].low){pl=MathMin(pl,m1[i].low);haveSwingL=true;}
     }
   if(!haveSwingH||!haveSwingL)
     {
   ...
   if(S.state==SETUP_CONFIRMED)
     {
      s.valid=true;
      s.reason="CONFIRMED_WAITING_ORIGIN_RETEST";
      ...
      if(s.valid)
        {
         ...
         S.originHigh=m1[1].high;S.originLow=m1[1].low;S.originClose=m1[1].close;
         ...
         Emit("SETUP_LOCATED",StringFormat(
              ...
              "\"originClose\":%.5f,\"originBarTime\":%I64d,\"extreme\":%.5f,\"bosKind\":\"%s\"",
              S.id,S.dir,S.originHigh,S.originLow,S.originClose,(long)S.originBarTime,S.extreme,S.bosKind));
        }
     }
   if(S.originHigh>S.originLow)
     {
      double px=s.dir<0?SymbolInfoDouble(_Symbol,SYMBOL_BID):SymbolInfoDouble(_Symbol,SYMBOL_ASK);
      s.inLocation=(s.dir<0)?(px<=S.originHigh&&px>=S.originLow)
                            :(px>=S.originLow&&px<=S.originHigh);
     }
...
void Manage()
  {
   ...
   double refPrice=(campOriginClose>0?campOriginClose:S.triggerPrice);
   datetime trigBar=a.triggerBarTime;
   double addOH=(campOriginHigh>0?campOriginHigh:S.originHigh);
   double addOL=(campOriginLow>0?campOriginLow:S.originLow);
