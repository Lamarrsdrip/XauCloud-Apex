...
//|                                                                   |
//|  Sizing: BOTH profiles use one 15/50/100 ladder of CURRENT        |
//|  executable capacity, re-derived on every layer. The profile      |
//|  still decides how capacity is established -- UNLIMITED keeps its |
//|  broker/server-proven capacity semantics untouched.               |
//+------------------------------------------------------------------+
#property copyright "XauCloud Apex"
#property version   "3.870"
#property strict
#property description "XauCloud Apex v3.8.7 UnifiedMarginLadder"

#include <Trade/Trade.mqh>
CTrade trade;

#define APEX_VERSION       "XauCloud-Apex_v3.8.7-UnifiedMarginLadder"
#define APEX_BUILD_ID      "3.8.7"
...
//====================== layer submission ==============================
// v3.8.7 UnifiedMarginLadder: BOTH profiles use the SAME layer percentage ladder --
// L1 15%, L2 50%, L3+ 100%. The percentage is always taken from the CURRENT executable
// capacity, which ComputeVolume() re-derives from live free margin / server evidence on
// EVERY layer; it is never a percentage of the starting balance, of SYMBOL_VOLUME_MAX,
// or of the capacity that existed when the campaign began.
//
// The PROFILE still decides how that capacity is established, and that is unchanged:
//   NORMAL    -- trusted broker margin economics, or the owner's reference leverage.
//   UNLIMITED -- the broker/server's own proven executable capacity, no reference
//                leverage, no invented lot cap. UNLIMITED remains UNLIMITED.
// Only the ladder is shared.
//
// Retired for sizing: C.baseMarginPct * C.layerMultiplier^layers. With the shipped
// defaults (100 and 2) that returned MathMin(100, 100*2^0) = 100% at L1, which is the
// root cause of UNLIMITED committing its entire capacity to the first entry. Both
// fields remain parsed for config compatibility but no longer size a layer.
   ...
   while(true)
     {
      ...
      // Upper bound on truth: strictly below the refused volume, and never above what
      // the server has already proven on this account+symbol.
      double capHi=FloorToStep(vol-d.volStep);
      ...
      if(reSized<d.volMin||reSized<=0||reSized>=vol)
        {
         PrintFormat("APEX SIZING ABORTED | profile=%s | %.2f%% of re-derived capacity %.4f is not executable",
                     ExecutionProfile(),pct,trueCap);
         break;
        }
      PrintFormat("APEX CAPACITY RE-DERIVED | profile=%s rejected=%.4f -> trueCapacity=%.4f -> %.2f%% = %.4f",
                  ExecutionProfile(),vol,trueCap,pct,reSized);
      Emit("SIZING_CAPACITY_REDERIVED",StringFormat(
         ",\"attempt\":%d,\"rejectedVolume\":%.4f,\"trueCapacity\":%.4f,\"marginPct\":%.2f,"
         "\"nextVolume\":%.4f,\"profile\":\"%s\",\"why\":\"%s\"",
         attempt+1,vol,trueCap,pct,reSized,ExecutionProfile(),why));
      vol=reSized;
      attempt++;
     }
