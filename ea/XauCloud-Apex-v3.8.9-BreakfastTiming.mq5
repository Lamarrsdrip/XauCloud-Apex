...
//|  hardening + v3.8.8 UnlimitedFromL3 sizing.                       |
//|  Same strategy: impulse -> sweep -> rejection -> micro BOS ->     |
//|  first probe on confirmation (if(s.valid) Start(s)) -> profit-    |
//|  side pyramiding -> basket exit.                                  |
//|                                                                   |
//|  v3.8.9 changes SETUP TIMING ONLY (does not touch lots/leverage): |
//|    - liquidity is a nearby swing pool, not a 70-bar rolling max   |
//|    - rejection = failed hold of the swept extreme (breakfast)     |
//|    - strong M1 displacement may bypass a lagging M3 colour gate   |
//|    - a confirm already >1.50 ATR from the extreme is not a trade  |
//|  Score stays 76. Lowering it would not have confirmed faster.     |
//|  v3.8.8 UNLIMITED sizing is unchanged.                            |
//+------------------------------------------------------------------+
#property copyright "XauCloud Apex"
#property version   "3.890"
#property strict
#property description "XauCloud Apex v3.8.9 BreakfastTiming"

#include <Trade/Trade.mqh>
CTrade trade;

#define APEX_VERSION       "XauCloud-Apex_v3.8.9-BreakfastTiming"
#define APEX_BUILD_ID      "3.8.9"
...
void EmitSetupTelemetry(const Snap &s,bool m3Available,bool m5Available,
                        bool m3Gate,bool m5Gate,double threshold)
  {
   ...
   Print("APEX SETUP | ",S.dir>0?"BUY":"SELL",
         ...
         " | m3=",C.requireM3Confirm?(m3Gate?"PASS":(s.m3Bypass?"BYPASS":"WAIT")):"OPTIONAL",
...
// v3.8.9 only changes WHEN that family becomes executable:
//   - liquidity is a nearby swing (rolling 9-40 fallback), not max(bars 9-79)
//   - rejection is a failed hold of the swept extreme, not a same-bar full
//     retrace through the pre-impulse prior
//   - strong M1 displacement may bypass a lagging M3 colour gate
//   - a confirm already extended from the extreme is refused
// 003/006/007 from v3.8.2 remain.
...
Snap Observe()
  {
   ...
   // Breakfast rejection: the market visited the swept extreme and then failed
   // to hold it. The old rule required ONE candle to both tag the extreme AND
   // close through the pre-impulse prior -- that is a full-impulse V-reversal
   // and is why live setups sat on "setup found" until the move was gone.
   int rb=MathMax(1,MathMin(C.rejectionBars,12));
   bool tagged=false,failedHold=false,closedThroughPrior=false;double bestW=0;
   for(int i=1;i<=rb;i++)
     {
      ...
      if(s.dir<0)
        {
         ...
         if(m1[i].close<S.extreme && m1[i].close<m1[i].open) failedHold=true;
         if(m1[i].close<S.prior) closedThroughPrior=true;
        }
      else
        {
   ...
   bool rej=tagged && (failedHold || closedThroughPrior);
   ...
   bool m3StronglyAgainst=false;
   if(m3Available && s.atr>0)
     {
      double m3Body=MathAbs(m3[1].close-m3[1].open);
      m3StronglyAgainst=s.dir<0
        ?(m3[1].close>m3[1].open && m3Body>0.5*s.atr)
        :(m3[1].close<m3[1].open && m3Body>0.5*s.atr);
     }
   bool m1Displacement=rej && bos && (bestW>=0.8 || s.impulseMult>=C.impulseAtr);
   s.m3Bypass=C.requireM3Confirm && m1Displacement && !m3StronglyAgainst &&
              !(s.m3Color&&(!InpRequireFreshM3||s.m3Fresh));
   bool m3Gate=(!C.requireM3Confirm)||(s.m3Color&&(!InpRequireFreshM3||s.m3Fresh))||s.m3Bypass;
...
int OnInit()
  {
   ...
   Print("APEX_READY ",APEX_VERSION,
      ...
      " | entry=confirm-then-start v3.8.2 + breakfast-timing v3.8.9");
