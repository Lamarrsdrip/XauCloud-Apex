...
const DEFAULT={armed:false,account:'0',symbolContains:'XAUUSD',targetMode:'MULTIPLIER',accountProfile:'NORMAL',targetEquity:1000,targetMultiplier:100,normalAccountMaxMultiplier:3,baseMarginPct:15,layerMultiplier:2,maxLayers:0,entryScore:76,addScore:70,impulseAtr:1.8,sweepAtr:.05,rejectionBars:5,watchExpiryMinutes:12,addSpacingAtr:.22,rejectionZoneAtr:.12,requireM3Confirm:true,requireM5Context:false,cooldownMinutes:0,learningEnabled:true,enableInvalidationExit:true,invalidationGivebackPct:.5,learningMinCampaigns:8,learningMaxScoreAdjustment:5};
...
export function clean(x={}){return {...DEFAULT,...x,armed:Boolean(x.armed),account:String(x.account??'0'),symbolContains:String(x.symbolContains||'XAUUSD').slice(0,32),targetMode:['MULTIPLIER','EQUITY'].includes(x.targetMode)?x.targetMode:'MULTIPLIER',accountProfile:['NORMAL','UNLIMITED'].includes(x.accountProfile)?x.accountProfile:'NORMAL',targetEquity:num(x.targetEquity,1000,.01,1e12),targetMultiplier:num(x.targetMultiplier,100,1.001,1e9),normalAccountMaxMultiplier:num(x.normalAccountMaxMultiplier,3,1.1,20),baseMarginPct:num(x.baseMarginPct,15,.1,100),layerMultiplier:num(x.layerMultiplier,2,1,10),maxLayers:Math.round(num(x.maxLayers,0,0,50)),entryScore:num(x.entryScore,76,40,100),addScore:num(x.addScore,70,40,100),impulseAtr:num(x.impulseAtr,1.8,.5,10),sweepAtr:num(x.sweepAtr,.05,0,2),rejectionBars:Math.round(num(x.rejectionBars,5,1,12)),watchExpiryMinutes:Math.round(num(x.watchExpiryMinutes,12,2,60)),addSpacingAtr:num(x.addSpacingAtr,.22,.02,5),rejectionZoneAtr:num(x.rejectionZoneAtr,.12,.02,2),requireM3Confirm:x.requireM3Confirm!==false,requireM5Context:Boolean(x.requireM5Context),cooldownMinutes:Math.round(num(x.cooldownMinutes,0,0,1440)),learningEnabled:x.learningEnabled!==false,enableInvalidationExit:x.enableInvalidationExit!==false,invalidationGivebackPct:num(x.invalidationGivebackPct,.5,.1,.95),learningMinCampaigns:Math.round(num(x.learningMinCampaigns,8,4,200)),learningMaxScoreAdjustment:num(x.learningMaxScoreAdjustment,5,0,15)}}
