...
test('SETUP-TELEMETRY: canonical and versioned v3.9.3 sources remain byte-identical',async()=>{
  const root=new URL('../',import.meta.url);
  const canonical=await fs.readFile(new URL('ea/XauCloud-Apex.mq5',root),'utf8');
  const versioned=await fs.readFile(new URL('ea/XauCloud-Apex-v3.9.3-ConfirmedDirection.mq5',root),'utf8');
